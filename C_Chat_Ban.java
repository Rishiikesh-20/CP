...
public class C_Chat_Ban {
    ...
    public static void main(String[] args) {
        ...
        testcase: while (t-- > 0) {
            ...
            if(k*(k+1)/2<x){
                long temp=x-k*(k+1)/2;
                ans=reversebinarySearch(k, temp);
                if(ans==-1)
                    System.out.println(k);
                else System.out.println(k+ans);
            }else{
                ans=binarySearch(k, x);
    ...
    public static long binarySearch(long k,long x){
        long low=1;
        long high=k;
        long ans=k;
        while(low<=high){
    ...
    public static long reversebinarySearch(long k,long x){
        long low=1;
        long high=k-1;
        long ans=k-1;
        while(low<=high){
            long mid=low+(high-low)/2;
            long val = mid * ((k-1) + (k-mid)) / 2;
            if(val>=x){
                ans=mid;
                high=mid-1;
            }else{
